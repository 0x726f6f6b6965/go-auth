...
import (
	...
	userService "github.com/0x726f6f6b6965/go-auth/internal/services"
...
func (s *UserAPI) Login(ctx *gin.Context) {
	...
	if err != nil {
		if errors.Is(err, userService.ErrDB) ||
			errors.Is(err, userService.ErrCreateToken) ||
			errors.Is(err, userService.ErrSalt) {
...
func (s *UserAPI) Register(ctx *gin.Context) {
	...
	if err != nil {
		...
		if errors.Is(err, userService.ErrRecordExist) {
