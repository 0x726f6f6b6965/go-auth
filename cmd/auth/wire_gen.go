...
func initUserAPI(ctx context.Context, ser v1.UserServiceServer) (*services.UserAPI, error) {
	servicesUserAPI := services.NewUserAPI(ser)
	return servicesUserAPI, nil
}

func initPolicyAPI(ctx context.Context, ser v1_2.PolicyServiceServer) (*services.PolicyAPI, error) {
	servicesPolicyAPI := services.NewPolicyAPI(ser)
	return servicesPolicyAPI, nil
}

func initGin(ctx context.Context, cfg *config.AppConfig) (*gin.Engine, error) {
	engine := InitGin(cfg)
	return engine, nil
}

func initPolicyService(ctx context.Context, cfg *config.AppConfig) (v1_2.PolicyServiceServer, func(), error) {
...
func initUserService(ctx context.Context, cfg *config.AppConfig, auth *jwtauth.JwtAuth) (v1.UserServiceServer, func(), error) {
...
func initAuth(ctx context.Context, cfg *config.AppConfig) *jwtauth.JwtAuth {
	jwtauthConfig := jwtAuthConfig(cfg)
	jwtAuth := jwtauth.NewJWTAuth(jwtauthConfig)
	return jwtAuth
}

func initPolicyGrpcClient(ctx context.Context, cfg *config.AppConfig) (v1_2.PolicyServiceClient, func(), error) {
